...
# class CVMPCA(nn.Module):
    ...
    def __init__(self,
        ...
        self.ray_pe = nn.Conv2d(3, embed_dim, kernel_size=1, stride=1, padding=0)

        # self.head = nn.Linear(embed_dim, n_classes, bias=False)

        _cls_head = nn.Linear(embed_dim, n_classes)
        _pos_head = nn.Sequential(
    ...
    def _single_view(self, feature_dict: Tensor) -> Dict[str, Dict[str, Tensor]]:
    ...
    def forward(self, required_rate, top_k, features: List[Dict[str, Tensor]]):
        visual_features = {
            camera_key: self._single_view(feature_dict)
        ...
        # make prediction from voxels
        output_list = [{key: head(voxel_features) for key, head in self.heads[0].items()}]

        # begin refinement from top-k proposals of the first prediction
        logits = output_list[0]['classification']
        ...
        # selecting coordinate of top-k voxels
        points = voxels_ #+ output_list[0]['position'].tanh()*self.voxel_size*3\
        output_list[0]['position'] = points
        points = points[torch.arange(B).unsqueeze(-1), top_proposals_index]

        # construct query: feature + class
        b_ids = torch.arange(B)
        selected_voxels = voxel_features[b_ids[..., None], top_proposals_index]  # (B, K, D)
        # print(logits[b_ids[..., None], top_proposals_index].shape, self.heads[0]['classification'].weight.shape,  self.heads[0]['classification'].bias.shape)
        ...
        for idx, (block, head_dict) in enumerate(zip(self.blocks, self.heads[1:])):
            ...
            query = block(query, points, feature)

            output_dict = {key: head(query) for key, head in head_dict.items()}
            points = points.detach() + output_dict['position'].tanh()*self.voxel_size*3
            output_dict['position'] = points 
        ...
        return {
            "voxels": voxels,
            "voxel_logits": logits,
            "top_indices": top_proposals_index,
    ...
    def _match(self, output_list, labels):
        ...
        for idx, output_dict in enumerate(output_list):
            ...
            pred_pos = output_dict['position']
            pred_link = output_dict['link']  # (B, K, C/3/M)
            # print(pred_link.shape, tgt_link.shape)
            ...
            # print("loss_link", loss_link.shape)
            ...
            # print(pred_link.flatten(0, 1).shape, tgt_link.shape, loss_link.shape)

            C = log_scores + .8*loss_pos + .4*loss_link
            C = C.view(B, K, -1).cpu()
    ...
    def training_step(self, batch, batch_idx):
        ...
        output_dict = self(num_query, image_dict)
        ...
        loss = 0
        level = batch_idx // 98 + 1
        loss_ces, loss_poses, loss_links = [], [], []
        for abc, (output, indices) in enumerate(zip(output_dict['output_list'], index_list)):
            ...
            coeff = float(level > abc)
            # print("coeff", level, abc, coeff)
            loss += coeff*(loss_ce + loss_link + 0.4*loss_pos)
            # break
        # print("-"*30)

        # print(pred_cls.shape, target_classes.shape)
        # print(pred_link.shape, target_link.shape)
        # acc_cls = ((pred_cls>0.5)*1. == target_classes).float().mean()
        # acc_link = ((pred_link[idx]>0.5)*1. == target_link).float().mean()
        # print(loss_pos.item())

        self.log("train_loss", loss, prog_bar=True)
        self.log("train_ce", loss_ces[min(level, self.n_layers-1)-1], prog_bar=True)
        self.log("train_pos", loss_poses[min(level, self.n_layers-1)-1], prog_bar=True)
        self.log("train_link", loss_links[min(level, self.n_layers-1)-1], prog_bar=True)
